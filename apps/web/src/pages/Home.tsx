...
export default function Home() {
  ...
  const weather = useMemo(pickRandomWeather, []);
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#fff4f8] text-slate-900">
      ...
      <section className="relative z-10 flex min-h-screen flex-col px-5 pb-6 pt-5 sm:px-8">
              ...
              <div className="grid grid-cols-[1.05fr_0.95fr] gap-3 pt-9">
                <TimeWidget now={now} username={user?.nickname || user?.username || "欢迎回来"} />
...
function TimeWidget({ now, username }: { now: Date; username: string }) {
  return (
    <section className="min-h-[150px] rounded-[32px] border border-white/68 bg-white/42 px-4 py-4 shadow-[12px_16px_40px_rgba(148,163,184,0.18),-12px_-16px_36px_rgba(255,255,255,0.82),inset_0_1px_0_rgba(255,255,255,0.72)] backdrop-blur-2xl">
      <p className="text-[12px] font-medium tracking-[-0.01em] text-slate-500/78">{formatShortDateLabel(now)}</p>
      <h1 className="mt-3 text-[42px] font-semibold leading-none tracking-[-0.075em] text-slate-800/90">
        {formatClock(now)}
      </h1>
      <p className="mt-4 max-w-[128px] truncate text-xs text-slate-500/76">{username}</p>
    </section>
  );
}
...
function WeatherWidget({ weather }: { weather: Weather }) {
  ...
  return (
    <section className="relative min-h-[150px] overflow-hidden rounded-[32px] border border-white/72 bg-white/42 p-4 shadow-[14px_18px_44px_rgba(148,163,184,0.18),-12px_-16px_36px_rgba(255,255,255,0.82),inset_0_1px_0_rgba(255,255,255,0.72)] backdrop-blur-2xl">
      ...
      <div className="relative flex h-full flex-col justify-between">
        <div>
          ...
          <div className="mt-2 flex items-center justify-between gap-2">
            <h2 className="text-[38px] font-semibold leading-none tracking-[-0.07em] text-slate-800/88">{weather.temperature}°</h2>
            <div className={`flex h-[48px] w-[48px] items-center justify-center rounded-[22px] text-white shadow-[8px_10px_22px_rgba(148,163,184,0.22),-6px_-8px_18px_rgba(255,255,255,0.86)] ${weather.iconClass}`}>
              <Icon className="h-7 w-7" />
            </div>
          </div>
          <p className="mt-2 truncate text-xs text-slate-500/78">{weather.label} · {weather.tip}</p>
        </div>
        <div className="mt-3 flex items-center gap-1.5 text-[10px] text-slate-500/68">
          <span className="rounded-full border border-white/62 bg-white/38 px-2 py-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)]">体感 {weather.feelsLike}°</span>
          <span className="truncate rounded-full border border-white/62 bg-white/38 px-2 py-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)]">{weather.wind}</span>
        </div>
      </div>
    </section>
  );
  ...
  const options: Weather[] = [
    {
      label: "晴",
      city: "当前位置",
      temperature: randomBetween(23, 31),
      feelsLike: randomBetween(24, 33),
      wind: "微风",
      tip: "适合出门走走",
    ...
    },
    {
      label: "雨",
      city: "当前位置",
      temperature: randomBetween(17, 24),
      feelsLike: randomBetween(16, 23),
      wind: "东风 2 级",
      tip: "记得带伞",
    ...
    },
    {
      label: "雪",
      city: "当前位置",
      temperature: randomBetween(-6, 2),
      feelsLike: randomBetween(-9, 0),
      wind: "西北风 3 级",
      tip: "路面湿滑",
    ...
    },
    {
      label: "夜间",
      city: "当前位置",
      temperature: randomBetween(15, 22),
      feelsLike: randomBetween(14, 21),
      wind: "晚风",
      tip: "适合安静聊天",
